 ...
 * @file __WinHook.c2v2.ahk
 * @author 
 * @date 2023/09/30
 * @version 0.0.0
 * @example see the link for an example
...
EWinHook_SetWinEventHook(eventMin, eventMax, hmodWinEventProc, lpfnWinEventProc, idProcess, idThread, dwflags) {
    ...
    ; dwflags check
    If ( !RegExMatch(dwflags, "S)^\s*(WINEVENT_(?:INCONTEXT|OUTOFCONTEXT))\s*\|\s*(WINEVENT_SKIPOWN(?:PROCESS|"                    . "THREAD))[^\S\n\r]*$|^\s*(WINEVENT_(?:INCONTEXT|OUTOFCONTEXT))[^\S\n\r]*$", &dwfArray) )
        Return 0
    dwflags := (dwfArray[1] && dwfArray[2]) ? %dwfArray[1]% | %dwfArray[2]% : %dwfArray[3]%
    ...
    If ( (lpfnWinEventProc) )
        lpfnWinEventProc := CallbackCreate(%lpfnWinEventProc%)
