 ...
 * @date          2023/09/25
 * @version       1.0.0
 * @ahkversion    2.0.2 (Unicode)
 * @osversion     Win 10 Pro (x64)
 ***********************************************************************/
; ---------------------------------------------------------------------------
;  Change History:
...
Class RichEdit {
	...
	; ---------------------------------------------------------------------------
	; @i...: RichEdit subclassproc
	; ---------------------------------------------------------------------------
	Static SubclassProc(H, M, W, L, I, R) { 
	...
	__New(GuiObj, Options, MultiLine := True) {
		Static   WS_TABSTOP := 0x10000,        WS_HSCROLL := 0x100000,
				WS_VSCROLL := 0x200000,       WS_VISIBLE := 0x10000000,
				WS_CHILD := 0x40000000,       WS_EX_CLIENTEDGE := 0x200,
				WS_EX_STATICEDGE := 0x20000,  ES_MULTILINE := 0x0004,
				ES_AUTOVSCROLL := 0x40,       ES_AUTOHSCROLL := 0x80,
				ES_NOHIDESEL := 0x0100,       ES_WANTRETURN := 0x1000,
				ES_DISABLENOSCROLL := 0x2000, ES_SUNKEN := 0x4000,
				ES_SAVESEL := 0x8000,         ES_SELECTIONBAR := 0x1000000
		; ---------------------------------------------------------------------------
		; @i...: RichEdit v4.1+ (Unicode)
		; ---------------------------------------------------------------------------
		Static MSFTEDIT_CLASS := "RICHEDIT50W"
		; ---------------------------------------------------------------------------
		; @i...: Specify default styles & exstyles
		; ---------------------------------------------------------------------------
		Styles := WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL
		If (MultiLine) {
			Styles |= WS_HSCROLL | WS_VSCROLL | ES_MULTILINE | ES_AUTOVSCROLL | ES_NOHIDESEL | ES_WANTRETURN | ES_DISABLENOSCROLL | ES_SAVESEL ; | ES_SELECTIONBAR ; does not work properly
			}
		...
		; This.RE.Add('Custom',CtrlOpts)
		; @i...: Initialize control
		...
		; ---------------------------------------------------------------------------
		; @i...: Subclass the control to get Tab key and prevent Esc from sending a WM_CLOSE message to the parent window.
		; ---------------------------------------------------------------------------
		; @i...: One of majkinetor's splendid discoveries!
		; ---------------------------------------------------------------------------
		DllCall("SetWindowSubclass", "Ptr", This.HWND, "Ptr",
				RichEdit.SubclassCB, "Ptr", This.HWND, "Ptr", 0)
		...
		; ---------------------------------------------------------------------------
		; @i...: Additional settings for multiline controls
		; @i...: Adjust the formatting rectangle
		; ---------------------------------------------------------------------------
		If (MultiLine) {
			RC := This.GetRect()
			This.SetRect(RC.L + 6, RC.T + 2, RC.R, RC.B)
			; ---------------------------------------------------------------------------
			; @i...: Set advanced typographic options
			; @i...: EM_SETTYPOGRAPHYOPTIONS = 0x04CA (WM_USER + 202)
			; @i...: TO_ADVANCEDTYPOGRAPHY = 1, TO_ADVANCEDLAYOUT = 8 ? not documented
			; ---------------------------------------------------------------------------
			SendMessage(0x04CA, 1, 8, This.HWND)
		}
		; ---------------------------------------------------------------------------
		; @i...: Correct AHK font size setting, if necessary
		; ---------------------------------------------------------------------------
		...
		; ---------------------------------------------------------------------------
		; @i...: Initialize the print margins
		; ---------------------------------------------------------------------------
		This.GetMargins()
		; ---------------------------------------------------------------------------
		; @i...: Initialize the text limit
		; ---------------------------------------------------------------------------
	...
	; @i...: PUBLIC METHODS
	; ---------------------------------------------------------------------------
	; @i...: Methods to be used by advanced users only
	; @i...: Retrieves the character formatting of the current selection
	; @i...: For details see http://msdn.microsoft.com/en-us/library/bb787883(v=vs.85).aspx
	; @i...: Returns a 'CF2' object containing the formatting settings
	; @MSDN_WIN32...: EM_GETCHARFORMAT := 0x043A
	; ---------------------------------------------------------------------------
	GetCharFormat() {
		EM_GETCHARFORMAT := 0x043A
		CF2 := RichEdit.CHARFORMAT2()
		SendMessage(EM_GETCHARFORMAT, 1, CF2.Ptr, This.HWND)
		Return (CF2.Mask ? CF2 : False)
	}
	; ---------------------------------------------------------------------------
	; @i...: Sets character formatting of the current selection
	; @i...: For details see http://msdn.microsoft.com/en-us/library/bb787883(v=vs.85).aspx
	; @i...: CF2 : CF2 object like returned by GetCharFormat()
	; @MSDN_WIN32...: EM_SETCHARFORMAT = 0x0444
   	; ---------------------------------------------------------------------------
	SetCharFormat(CF2) {
			Return SendMessage(0x0444, 1, CF2.Ptr, This.HWND)
	}
   ; ---------------------------------------------------------------------------
   ; @i...: Retrieves the paragraph formatting of the current selection
   ; @i...: For details see http://msdn.microsoft.com/en-us/library/bb787942(v=vs.85).aspx
   ; @i...: Returns a 'PF2' object containing the formatting settings
   ; @i...: EM_GETPARAFORMAT := 0x043D
   ; ---------------------------------------------------------------------------
   GetParaFormat() { 
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Sets the  paragraph formatting for the current selection
   ; @i...: For details see http://msdn.microsoft.com/en-us/library/bb787942(v=vs.85).aspx
   ; @i...: PF2 : PF2 object like returned by GetParaFormat()
   ; @i...: EM_SETPARAFORMAT := 0x0447
   ; ---------------------------------------------------------------------------
   SetParaFormat(PF2) { 
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Control specific
   ; @i...: Has the control been modified (?)
   ; @i...: EM_GETMODIFY = 0xB8
   ; ---------------------------------------------------------------------------
   IsModified() { 
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Sets or clears the modification flag for an edit control
   ; @i...: EM_SETMODIFY := 0xB9
   ; ---------------------------------------------------------------------------
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Set the events which shall send notification codes controls owner
   ; @i...: Events : Array containing one or more of the keys defined in 'ENM'
   ; @i...: For details see http://msdn.microsoft.com/en-us/library/bb774238(v=vs.85).aspx
   ; @i...: EM_SETEVENTMASK	= 	0x0445
   ; ---------------------------------------------------------------------------
   SetEventMask(Events?) { 
		Static ENM := {
			NONE: 0x00, CHANGE: 0x01, UPDATE: 0x02, SCROLL: 0x04, SCROLLEVENTS: 0x08, DRAGDROPDONE: 0x10, PARAGRAPHEXPANDED: 0x20, PAGECHANGE: 0x40, KEYEVENTS: 0x010000, MOUSEEVENTS: 0x020000, REQUESTRESIZE: 0x040000, SELCHANGE: 0x080000, DROPFILES: 0x100000, PROTECTED: 0x200000, LINK: 0x04000000
		}
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Loading and storing RTF format
   ; @i...: Gets the whole content of the control as rich text
   ; @i...: Selection = False : whole contents (default)
   ; @i...: Selection = True  : current selection
   ; @i...: EM_STREAMOUT = 0x044A
   ; @i...: SF_TEXT = 0x1, SF_RTF = 0x2, SF_RTFNOOBJS = 0x3, SF_UNICODE = 0x10, SF_USECODEPAGE =	0x0020
   ; @i...: SFF_PLAINRTF = 0x4000, SFF_SELECTION = 0x8000
   ; @i...: UTF-8 = 65001, UTF-16 = 1200
   ; ---------------------------------------------------------------------------
   GetRTF(Selection := False) { 
		; Static GetRTFCB := CallbackCreate(RichEdit_GetRTFProc)
		Flags := 0x4022 | (1200 << 16) | (Selection ? 0x8000 : 0)
		; ---------------------------------------------------------------------------
		; @i...: EDITSTREAM structure
		; ---------------------------------------------------------------------------
		ES := Buffer((A_PtrSize * 2) + 4, 0)                  
		; ---------------------------------------------------------------------------
		; @i...: dwCookie
		; ---------------------------------------------------------------------------
		NumPut("UPtr", This.HWND, ES)                         
		; ---------------------------------------------------------------------------
		; @i...: pfnCallback
		; ---------------------------------------------------------------------------
		NumPut("UPtr", RichEdit.GetRTFCB, ES, A_PtrSize + 4)  
		; SendMessage(0x044A, Flags, ES.Ptr, This.HWND)
		SendMessage(0x044A, SF_RTF:=0x2, ES.Ptr, This.HWND)
		Return RichEdit.GetRTFProc("*GetRTF*", 0, 0, 0)
   }
   ; ---------------------------------------------------------------------------
   ; @i...: Loads RTF file into the control
   ; @i...: FilePath = file path
   ; @i...: Selection = False : whole contents (default)
   ; @i...: Selection = True  : current selection
   ; @i...: EM_STREAMIN = 0x0449
   ; @i...: SF_TEXT = 0x1, SF_RTF = 0x2, SF_RTFNOOBJS = 0x3, SF_UNICODE = 0x10, SF_USECODEPAGE =	0x0020
   ; @i...: SFF_PLAINRTF = 0x4000, SFF_SELECTION = 0x8000
   ; @i...: UTF-16 = 1200
   ; ---------------------------------------------------------------------------
   LoadRTF(FilePath, Selection := False) { 
		...
		; @i...: dwCookie
		; ---------------------------------------------------------------------------
		NumPut("UPtr", File.Handle, ES)
		; ---------------------------------------------------------------------------
		; @i...: pfnCallback
		; ---------------------------------------------------------------------------
		NumPut("UPtr", RichEdit.LoadRTFCB, ES, A_PtrSize + 4)
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Scrolling
   ; @i...: Obtains the current scroll position
   ; @i...: Returns on object with keys 'X' and 'Y' containing the scroll position
   ; @i...: EM_GETSCROLLPOS := 0x04DD
   ; ---------------------------------------------------------------------------
   GetScrollPos() { 
		PT := Buffer(8, 0)
   ...
   ; ---------------------------------------------------------------------------
	; @i...: Scrolls the contents of a rich edit control to the specified point
	; @i...: X : x-position to scroll to
	; @i...: Y : y-position to scroll to
	; @i...: EM_SETSCROLLPOS := 0x04DE
	; ---------------------------------------------------------------------------
   SetScrollPos(X, Y) { 
		PT := Buffer(8, 0)
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Scrolls the caret into view
   ; @i...: EM_SCROLLCARET := 0x00B7
   ; ---------------------------------------------------------------------------
   ScrollCaret() { 
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Shows or hides one of the scroll bars of a rich edit control
   ; @i...: SB 	 : Identifies which scroll bar to display: horizontal or vertical
   ; @i...: This parameter must be 1 (SB_VERT) or 0 (SB_HORZ)
   ; @i...: Mode : Specify TRUE to show the scroll bar and FALSE to hide it
   ; @i...: EM_SHOWSCROLLBAR = 0x0460 (WM_USER + 96)
   ; ---------------------------------------------------------------------------
   ...
   ; ---------------------------------------------------------------------------
   ; @i...: Text and selection
   ; @i...: Finds Unicode text within a rich edit control
   ; @i...: Find : Text to search for
   ; @i...: Mode : Optional array containing one or more of the keys specified in 'FR'
   ; @i...: For details see http://msdn.microsoft.com/en-us/library/bb788013(v=vs.85).aspx
   ; @i...: Returns True if the text was found; otherwise false
   ; @i...: EM_FINDTEXTEXW = 0x047C, EM_SCROLLCARET = 0x00B7
   ; ---------------------------------------------------------------------------
	FindText(Find, Mode?) { 
	...
	; ---------------------------------------------------------------------------
	; @i...: Finds the next word break before or after the specified character position or retrieves information about
	; @i...: the character at that position.
	; @i...: CharPos : Character position.
	; @i...: Mode    : Can be one of the keys specified in 'WB'
	; @i...: Returns the character index of the word break or other values depending on 'Mode'
	; @i...: For details see http://msdn.microsoft.com/en-us/library/bb788018(v=vs.85).aspx
	; @i...: EM_FINDWORDBREAK = 0x044C (WM_USER + 76)
	; ---------------------------------------------------------------------------
	FindWordBreak(CharPos, Mode := "Left") { 
			Static WB := {
					LEFT: 0, RIGHT: 1, ISDELIMITER: 2, CLASSIFY: 3,
					MOVEWORDLEFT: 4, MOVEWORDRIGHT: 5, LEFTBREAK: 6,
					RIGHTBREAK: 7
			}
	...
	; ---------------------------------------------------------------------------
   	; @i...: Retrieves the currently selected text as plain text
	; @i...: Returns selected text
	; @i...: EM_GETSELTEXT := 0x043E, EM_EXGETSEL := 0x0434
	; ---------------------------------------------------------------------------
	GetSelText() { 
		Txt := ""
		CR := This.GetSel()
	...
	; @i...: Returns an object containing the keys S (start of selection) and E (end of selection))
	; @i...: EM_EXGETSEL := 0x0434
	; ---------------------------------------------------------------------------
	GetSel() { 
		CR := Buffer(8, 0)
	...
	; ---------------------------------------------------------------------------
	; @i...: Gets the whole content of the control as plain text
	; @i...: EM_GETTEXTEX = 0x045E
	; ---------------------------------------------------------------------------
	GetText() {  
		Txt := ""
	...
	; ---------------------------------------------------------------------------
	; @i...: Gets the text and background colors
	; @i...: not implemented
	; GetTextColors() { 
	; }
	; ---------------------------------------------------------------------------
	...
	; ---------------------------------------------------------------------------
	; @i...: Retrieves the position of the first occurence of the specified text within the specified range
	; @i...: Find : Text to search for
	; @i...: Min  : Character position index immediately preceding the first character in the range
	; @i...:        Integer value to store as cpMin in the CHARRANGE structure
	; @i...:        Default: 0 - first character
	; @i...: Max  : Character position immediately following the last character in the range
	; @i...:        Integer value to store as cpMax in the CHARRANGE structure
	; @i...:        Default: -1 - last character
	; @i...: Mode : Any combination of the following values
	; @i...:        0 : search backward, 1 : search forward
	; @i...: 		2 : match whole word only, 4 : case-sensitive
	; @i...: Returns an object containing the keys S (start of text) and E (end of text) if found, otherwise False
	; @i...: EM_FINDTEXTEXW = 0x047C
	; ---------------------------------------------------------------------------
	...
	; ---------------------------------------------------------------------------
	; @i...: Retrieves a specified range of characters from a rich edit control
	; @i...: Min : Character position index immediately preceding the first character in the range
	; @i...:       Integer value to store as cpMin in the CHARRANGE structure
	; @i...: Max : Character position immediately following the last character in the range
	; @i...:       Integer value to store as cpMax in the CHARRANGE structure.
	; @i...: CHARRANGE -> http://msdn.microsoft.com/en-us/library/bb787885(v=vs.85).aspx
	; @i...: EM_GETTEXTRANGE := 0x044B
	; ---------------------------------------------------------------------------
   ...
   SetOptions(Options, Mode := "SET") { ; Sets the options for a rich edit control
      ...
      Static ECO := {AUTOWORDSELECTION: 0x01, AUTOVSCROLL: 0x40, AUTOHSCROLL: 0x80, NOHIDESEL: 0x100, READONLY: 0x800, WANTRETURN: 0x1000, SAVESEL: 0x8000, SELECTIONBAR: 0x01000000, VERTICAL: 0x400000}, ECOOP := {SET: 0x01, OR: 0x02, AND: 0x03, XOR: 0x04}
   ...
   GetFont(Default := False) { ; Get current font
      ...
      CFS := CF2.Effects
	Style := (CFS & 1 ? "B" : "") . (CFS & 2 ? "I" : "") . (CFS & 4 ? "U" : "") . (CFS & 8 ? "S" : "") 	. (CFS & 0x10000 ? "L" : "") . (CFS & 0x20000 ? "H" : "") . (CFS & 16 ? "P" : "")
   ...
   Print() {
      ...
      ; Static variables
      ; ---------------------------------------------------------------------------
      Static PD_ALLPAGES := 0x00, PD_SELECTION := 0x01, PD_PAGENUMS := 0x02, PD_NOSELECTION := 0x04, PD_RETURNDC := 0x0100, PD_USEDEVMODECOPIES := 0x040000, PD_HIDEPRINTTOFILE := 0x100000, PD_NONETWORKBUTTON := 0x200000, PD_NOCURRENTPAGE := 0x800000, MM_TEXT := 0x1, DocName := "AHKRichEdit", PD_Size := (A_PtrSize = 8 ? (13 * A_PtrSize) + 16 : 66)
      ErrorMsg := ""
      ; ---------------------------------------------------------------------------
      ...
      Sel := This.GetSel()
      ; Determine/Set Flags
      Flags := PD_ALLPAGES | PD_RETURNDC | PD_USEDEVMODECOPIES | PD_HIDEPRINTTOFILE | PD_NONETWORKBUTTON | PD_NOCURRENTPAGE
      If (Sel.S = Sel.E)
      ...
      ; Programming note: The values that are loaded to these fields are critical. The Print dialog will not display (returns an error) if unexpected values are loaded to one or more of these fields.
      ; ---------------------------------------------------------------------------
      ; Print dialog box
      ...
      ; ---------------------------------------------------------------------------
      ; function ...:   Determine From/To Page
      ; ---------------------------------------------------------------------------
      ...
      Else
         PageF := 1, PageL := 65535
      ; ---------------------------------------------------------------------------
      ; function ...:   Collect printer capacities
      ; ---------------------------------------------------------------------------
      Caps := This.GetPrinterCaps(PDC)
