...
/*[Class] WinHook by Fanatic Guru
; AHK v1 => Fanatic Guru [2019 02 18 v2]
;
...
;
;				Shell Hook Events:
;				1 = HSHELL_WINDOWCREATED := 1
;				2 = HSHELL_WINDOWDESTROYED := 2
;				3 = HSHELL_ACTIVATESHELLWINDOW := 3
;				4 = HSHELL_WINDOWACTIVATED := 4
;				32772 = HSHELL_WINDOWACTIVATED := 32772
;				5 = HSHELL_GETMINRECT := 5
;				6 = HSHELL_REDRAW := 6
;				7 = HSHELL_TASKMAN := 7
;				8 = HSHELL_LANGUAGE := 8
;				9 = HSHELL_SYSMENU := 9
;				10 = HSHELL_ENDTASK := 10
;				11 = HSHELL_ACCESSIBILITYSTATE := 11
;				12 = HSHELL_APPCOMMAND := 12
;				13 = HSHELL_WINDOWREPLACED := 13
;				14 = HSHELL_WINDOWREPLACING := 14
;				32768 = 0x8000 = HSHELL_HIGHBIT := 32768
;				32772 = 0x8000 + 4 = 0x8004 = HSHELL_RUDEAPPACTIVATED := 32772 
; 				(HSHELL_HIGHBIT + HSHELL_WINDOWACTIVATED)
;				32774 = 0x8000 + 6 = 0x8006 = HSHELL_FLASH := 32774 
...
;				0x0009 = EVENT_SYSTEM_CAPTUREEND := 0x0009
;				0x0008 = EVENT_SYSTEM_CAPTURESTART := 0x0008
...
class WinHook {
	class Shell {
		static Add(Func, wTitle := "", wClass := "", wExe := "", Event := 0) {
			...
			WinHook.Shell.Events.Set(Event, true)
		...
		static UnHook(Index) {
			...
			WinHook.Shell.Events.Clear()
			Loop WinHook.Shell.Hooks.Length
				if WinHook.Shell.Hooks.Has(A_index)
					WinHook.Shell.Events.Set(WinHook.Shell.Hooks[A_index].Event, true)
		}
		static UnHookAll() {
			DllCall("DeregisterShellHookWindow", "UInt", A_ScriptHwnd)
			WinHook.Shell.Hooks := Array(), WinHook.Shell.Events.Clear()
		}
		static Message(Event, Hwnd, *) { ; Private Method
			DetectHiddenWindows(True)
			If WinHook.Shell.Events.Has(Event) or WinHook.Shell.Events.Has(0) {
				try wTitle := WinGetTitle("ahk_id " Hwnd)
				try wClass := WinGetClass("ahk_id " Hwnd)
				try wExe := WinGetProcessName("ahk_id " Hwnd)
				for key, Hook in WinHook.Shell.Hooks
					try if ((Hook.Title = wTitle or Hook.Title = "") and (Hook.Class = wClass or Hook.Class = "") and (Hook.Exe = wExe or Hook.Exe = "") and (Hook.Event = Event or Hook.Event = 0))
	...
	class Event {
		static Add(eventMin, eventMax, eventProc, idProcess := 0, WinTitle := "") {
			...
			if !IsObject(eventProc)
				eventProc := eventProc.Call()
			WinHook.Event.Hooks.Set(hWinEventHook, { eventMin: eventMin, eventMax: eventMax, eventProc: eventProc, idProcess: idProcess, WinTitle: WinTitle })
		...
		static UnHookAll(*) {
			if WinHook.Event.HasOwnProp("Hooks") {
				for hWinEventHook, Hook in WinHook.Event.Hooks
					DllCall("UnhookWinEvent", "Ptr", hWinEventHook)
				WinHook.Event.Hooks.Clear(), CB_WinEventProc := ""
			}
		}
